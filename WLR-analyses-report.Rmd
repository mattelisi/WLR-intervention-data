...
## Group comparison of TEC at T1

```{R, echo=F}

nd <- data.frame(TEC=c(d$TEC), 
                 id = rep(d$ID,1),
                 nationality = rep(d$nationality,1),
                 wlr_group = rep(d$wlr_group,1))
nd$wlr_group <- ifelse(nd$wlr_group=="C","Control","WLR")
nd$wlr_group <- factor(nd$wlr_group)

#any(is.na(nd$nationality))
#any(is.na(nd$wlr_group))

#with(nd, tapply(TEC, list(nationality, wlr_group), mean))
#with(nd, tapply(TEC, list(nationality, wlr_group), sd))

# full model
library(rankFD)
m.TEC <- rankFD(TEC~wlr_group*nationality, data=nd)
#print(m.TEC)

kable(with(nd, tapply(TEC, list(nationality, wlr_group), mean)), caption = "Mean TEC scores at T1 by group",digits=2)
```

```{R, echo=F}
kable(with(nd, tapply(TEC, list(nationality, wlr_group), sd)), caption = "Std TEC scores at T1 by group",digits=2)
```

```{R, echo=F}

kable(m.TEC$ANOVA.Type.Statistic , caption = "Insecurity: ANOVA-Type Statistc. TEC at T1 time point",digits=6)
```
