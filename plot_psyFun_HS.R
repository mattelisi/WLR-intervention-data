...
plotHS <- ggplot(dag2[dag2$time!="T3",], aes(x=img_number,y=classification, color=group, fill=group))+geom_vline(xintercept=11,color="grey",size=0.2,lty=2)+geom_hline(yintercept=0.5,color="grey",size=0.2,lty=2)+geom_errorbar(data=dag2[dag2$time!="T3",],aes(ymin=classification-se,ymax=classification+se),width=0)+geom_point(data=dag2[dag2$time!="T3",],pch=21,fill=NA)+nice_theme+scale_x_continuous(breaks= x_breaks, labels=x_labels,limits=range(d$img_number))+labs(x="morph level",y="choice probability")+scale_color_manual(values=c("dark grey","dark green"),name=" ")+scale_fill_manual(values=c("dark grey","dark green"),name=" ")+facet_grid(ethni~time)+ theme(panel.spacing.x = unit(1, "lines"))+geom_line(data=d_psy[d_psy$time!="T3",],aes(y=P),size=0.8)+geom_ribbon(data=d_psy[d_psy$time!="T3",],aes(ymin=P_lb, ymax=P_ub), alpha = 0.3, size = 0, color=NA)


ggsave("./figures/psy_HS.pdf",plotHS, width=5,height=2.5)
...
ggsave("./figures/CIdiff_HS.pdf",diff_plot, width=2.5,height=2)

# HPDI(i_mu - i_mu_T1, prob=0.95)[1]
# library(bayesplot)
